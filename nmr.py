...
def nmr_loop(target, l, ca, cb, cc, cdamp):
	...
	def _calc(sa, sb, sc):
		calc = nmr_probe_tune(l, sa, sb, sc)
		l_match_required = nmr_l_match(target, sa, sb, sc)
...
def nmr_l_match(f, ca, cb, cc):
	...
	xb = _xcap(cb)
	xc = _xcap(cc)
	return 2*-1*((xa+2*xb)*xc)/(4*math.pi*f*(xa+2*xb+xc))
